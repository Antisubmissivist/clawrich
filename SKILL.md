...
name: clawrich
version: 0.1.1
description: ClawRich ‚Ä?Telegram Bot API 10.1 Rich Message sender for OpenClaw. True tables, interactive checklists, collapsible details, math, custom emoji. Bypasses OpenClaw's HTML-only text path.
...
configuration ‚Ä?**no new credentials needed**. Make sure you have a Telegram
account configured:
...
{
  "chat_id": "6462079744",
  "heading": "üìä Sprint Status",
  "summary": "Driver App shipped. Portal QA in progress. Route Optimizer blocked.",
  "table": {
    "columns": ["Task", "Owner", "Status"],
    "rows": [
      ["Driver App release", "Alex", "‚ú?Done"],
      ["Portal QA", "Sam", "üîÑ In progress"],
      ["Route optimizer", "Luke", "üö´ Blocked"]
    ]
  },
  "list": [
...
- Free users see checklist items as static text ‚Ä?tables and details still render
...
- Inline buttons (`reply_markup`) are not yet wrapped ‚Ä?use OpenClaw's native `message`
...
- Structured input (heading/summary/table/list/details) ‚Ü?HTML string
